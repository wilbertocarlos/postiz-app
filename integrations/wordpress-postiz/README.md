...
workflow publishes the same immutable Linux/AMD64 tag to GHCR for the Coolify
server. Configure Coolify with that exact tag, never `latest`. Rollback means
restoring the previous tag and redeploying; no database migration is involved.
